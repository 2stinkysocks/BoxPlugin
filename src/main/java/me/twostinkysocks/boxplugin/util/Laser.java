...
//        // --- Add this block for X/Z offset correction --- may or may not work idk
//        Vector dir = this.end.toVector().subtract(this.start.toVector()).normalize();
//        Vector perpOffset = new Vector(-dir.getZ(), 0, dir.getX()).multiply(0.08); // tweak 0.01 if needed
//        this.start.add(perpOffset);
    ...
    public static class GuardianLaser extends Laser {
        ...
        protected void sendStartPackets(Player p, boolean hasSeen) throws ReflectiveOperationException {
            Object guardianSpawn = getGuardianSpawnPacket();
            Object squidSpawn = squid == null ? null : getSquidSpawnPacket();

            // NEW: force precise position immediately after spawn
            Object guardianMove = Packets.createPacketMoveEntity(guardian);
            Object squidMove = squid == null ? null : Packets.createPacketMoveEntity(squid);
