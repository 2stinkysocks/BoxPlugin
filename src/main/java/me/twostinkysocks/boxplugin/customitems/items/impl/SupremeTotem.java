...
public class SupremeTotem extends CustomItem {
    private HashMap<UUID, Long> cooldown;

    public SupremeTotem(CustomItemsMain plugin) {
        super(
                ChatColor.GOLD + "Supreme Totem",
                "Supreme_TOTEM",
    ...
    public void totemPop(Player p){
        p.setHealth(Math.min(p.getAttribute(Attribute.MAX_HEALTH).getValue(), 40));
        p.addPotionEffect(new PotionEffect(PotionEffectType.REGENERATION, 100, 3, true, false));
        p.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION, 180, 6, true, false));
    }
}
