...
public class PerkSpeed extends AbstractPerk implements Upgradable {
    public PerkSpeed() {
        setCost(3);
    ...
    //level 1 returns speed 3, increment by 1 per level
    @Override
    ...
    public int getNextCost(int currentLevel) {
        int cost = 0;
        if(currentLevel == 1){
            cost = (int) (16 * 64); //16 teracubes
        } else if (currentLevel == 2) {
            cost = (int) (16 * 64 * 16); //4 hexidium
        } else if (currentLevel == 3) {
            cost = (int) (16 * 64 * 16 * 4); //16 hexidium
        }
        return cost;
    }
